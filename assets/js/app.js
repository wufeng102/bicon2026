...
function pubCard(p, i) {
  return '<div class="card pub rv' + (p.n === 5 ? ' pub--wide' : '') + '" data-d="' + (i % 4) + '"><div class="pub__n" style="background:' + p.color + '">' + p.n + '</div><div><div class="pub__t">' + esc(p.name) + '</div><span class="chip pub__i ' + (p.n === 5 ? 'chip--g' : '') + '">' + esc(p.index) + '</span>' +
    (p.restricted ? '<div class="tl__n" style="margin-top:8px;font-style:italic">' + esc(L('Available only for authors not affiliated with Poltekkes Kemenkes Bengkulu', 'Hanya untuk penulis yang tidak berafiliasi dengan Poltekkes Kemenkes Bengkulu')) + '</div>' : '') +
    (p.websiteUrl ? '<div style="margin-top:8px"><a href="' + esc(p.websiteUrl) + '" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:6px;font-size:13.5px;font-weight:700">' + esc(L('Visit journal website', 'Kunjungi situs jurnal')) + icon('ext') + '</a></div>' : '') + '</div></div>';
}
